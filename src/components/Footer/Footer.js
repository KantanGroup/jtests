...
class Footer extends React.Component {
  render() {
    ...
    if (countryCode) {
      ...
      menuGame = (
        <Col md={3}>
          <div className={s.footer}><span className={s.text}>Game trend in {capitalize(countryName)}</span></div>
          {games.map((category, index) => (
            <div className={s.footer}>- <Link className={s.link} to={`/app-trend-in-${countryName.toLowerCase().split(' ').join('-')}/googlestore/top-app/${category.split('_').join('-')}-category`}>{capitalize(category.split('_').join(' '))}</Link></div>
          ))}
        </Col>
      );
      menuApp = (
        <Col md={3}>
          <div className={s.footer}><span className={s.text}>App trend in {capitalize(countryName)}</span></div>
