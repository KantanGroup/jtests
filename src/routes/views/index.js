  ...
  async action({ params }) {
    ...
    if (data.app.app) {
      ...
      return {
        ...
        component: <Layout countryCode={getCountryCode(countryName)}><Views app={app} trend={data.trend} similar={data.app.similar} appId={appId} countryCode={getCountryCode(countryName)} countryName={countryName} /></Layout>,
      };
    }
    return {
      title: `${appId} app trends in ${capitalize(countryName.split('-').join(' '))}`,
      description: `${appId} app trends in ${countryName.split('-').join(' ')}`,
      component: <Layout countryCode={getCountryCode(countryName)}><Views appId={appId} countryCode={getCountryCode(countryName)} countryName={countryName} /></Layout>,
    };
  },
