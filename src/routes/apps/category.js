...
export default {
  ...
  path: '/app-trend-in-:countryName/app/:appId/:categoryName-category',
  ...
  async action({ params }) {
    ...
    const resp = await fetch('/graphql', {
      ...
      body: JSON.stringify({
        query: `{app(appId:"${appId}",language:"${getLanguageCode(countryName)}"){app{appId,title,summary,descriptionHTML,developerId,developerEmail,developerWebsite,genre,version,playstoreUrl,video,point,price,minInstalls,maxInstalls,androidVersion,updated},similar{apps{appId,title,developer{devId},icon,score,price}}},trend(appId:"${appId}",countryCode:"${getCountryCode(countryName)}",category:"all"){topgrossing{index,createAt}topsellingFree{index,createAt}topsellingPaid{index,createAt}topsellingNewFree{index,createAt}topsellingNewPaid{index,createAt}},category(appId:"${appId}",countryCode:"${getCountryCode(countryName)}",category:"${categoryName.split('-').join('_')}"){topgrossing{index,createAt}topsellingFree{index,createAt}topsellingPaid{index,createAt}topsellingNewFree{index,createAt}topsellingNewPaid{index,createAt}}}`,
      }),
      credentials: 'include',
    });
